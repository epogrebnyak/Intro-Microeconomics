    ...
    def plot(
        ...
        if label == True:
            ...
            if type(self).__name__ == "Demand":
                x0 = self.q_intercept * 0.95
            else:
                x0 = ax.get_xlim()[1] * 0.9
    ...
    def plot_surplus(self, p, ax=None):
        ...
        if p <= self.intercept:
            cs_plot = ax.fill_between(
                [0, self.q(p)], y1=[self.intercept, p], y2=p, alpha=0.1
            )
...
class Supply(Affine):
    ...
    def plot_surplus(self, p, ax=None):
        ...
        if q > self.q_intercept > 0:
            rectangle_plot = ax.fill_between(
                [0, qq, q], y1=[0, 0, p], y2=p, color="C1", alpha=0.1
            )
