...
class Equilibrium:
    ...
    def plot_clean(self, ax=None, fresh_ticks=True, axis_arrows=True):
        ...
        if fresh_ticks:
            ...
            ax.set_xticks(
                sorted(list([round(x, 2) for x in combined_xticks if not np.isnan(x)]))
            )
            ax.set_yticks(
                sorted(list([round(x, 2) for x in combined_yticks if not np.isnan(x)]))
            )
        ...
        ax.text(q_val, self.demand.p(q_val) + y_delta * 0.99, "D", size=14)

        # add supply label
        y1 = self.supply.p(q_val)
        if y1 > ylims[1]:
            q_val = self.supply.q(ylims[1] * 0.95)
